...
namespace AppConfig_Demo
{
    class Program
    {
        ...
        {
            //总体说明：
            //1 : AppConfig操作的是Web项目的web.config 或win项目的app.config
            //2 : AppConfig操作的都是Connections节点和AppSettings节点，其它节点与此无关
            //3 ：AppConfig里的属性名称，都和 AppSettings 里的配置name一致。

            string providerName;
            string connectionString = AppConfig.GetConn("Conn", out providerName);//读取链接，同时返回providerName，框架从链接字符串里自动识别类型，所以一般不用配ProviderName
